...
## Anyone you worked with on this project: Rachel Richardson
...
def find_urls(s):
    #Your code here
    urls = re.findall('http[s]?://[^\s]+\.(?:[\/a-zA-Z]{2,}|[0-9]{2,}|[$-_@.&+]{2,}|[!*\(\),]{2,})+', s) 
    # this is finding all urls in a string by finding any characters matching this: 
    # https:// (the s is optional), includes /, \, $, @, etc., letters a-z, numbers 0-9, etc. (all the things in the [] brackets) and ends the url when it checks to see if there's two or more characters after a '.' and fails (so, there's only 1 character or none) 
    return urls
...
def grab_headlines():
    ...
    for level1 in soup.find('div', {'class': 'item-list'}): #looping through the levels in the html to get to the level that contains the Most Read headlines
        ol = soup.find('ol')
        for level2 in ol.find_all('li'):
                text = level2.a.get_text()
...
def get_umsi_data():
    ...
    get_access = requests.get(url, headers={'User-Agent': 'SI_CLASS'})
    soup = BeautifulSoup(get_access.text, "html.parser")

    names = []
    titles = []


    for x in range(13): # looping through all pages on the site to get all the faculty
        updated_url = url + ('&page='+ str(x))
        get_access = requests.get(updated_url, headers={'User-Agent': 'SI_CLASS'})
        soup = BeautifulSoup(get_access.text, "html.parser")

        for level1 in soup.find_all('div', {'class': 'field-item even'}, property = 'dc:title'):
            name = level1.h2.get_text()
            names.append(name)

        for level1 in soup.find_all('div', {'class': 'field field-name-field-person-titles field-type-text field-label-hidden'}):
            for level2 in level1.find_all('div', {'class': 'field-items'}):
                 for level3 in level2.find_all('div', {'class': 'field-item even'}):
                    title = level3.get_text()
                    titles.append(title)
